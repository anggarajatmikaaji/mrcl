...
\begin{itemize}
	...
	\item {\color{Orange}TODO: HDFS 사용 여부에 따라 달라질 수 있음} 계산에 필요한 2개의 matrix는 클러스터의 모든 노드가 각각 복사본을 저장하고 있다. 전체 노드 수의 log에 비례하는 시간 만에 모두 복사할 수 있으므로 matrix multiplication의 시간복잡도에 영향을 주지 않으므로 구현을 간단히 하기 위해 그러한 분산 저장 과정을 고려하지 않는다.
\end{itemize}

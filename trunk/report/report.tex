...
Matrix multiplication을 구현하는 방법은 여러 가지가 있는데, 정의대로 계산하는 $O(n^3)$ 짜리와 $O(n^{2.376})$의 시간이 걸리는 Strassen 알고리즘 등이 있으나 우리는 MapReduce에서 쓰기 알맞도록 outer product를 응용한 방식을 사용하였다.
